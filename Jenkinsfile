      ...
      parallel {
        stage('XtTools Copy') {
          steps {
            node(label: 'vlno-tenfe02-rel') {
              dir(path: '/usr/local/xrfs_s/xars/winXtDevTools/downloads') {
        ...
        stage('Windows Copy') {
          steps {
            node(label: 'vlno-tenfe02-rel') {
              ...
              dir(path: "/usr/local/xrfs_s/xars/winXtDevTools/downloads/$RELEASE_VERSION") {
              ...
              dir(path: "/usr/local/xrfs_s/xars/winWS/$PRECREATED_WS_BRANCH") {
        ...
        stage('Linux Copy') {
          steps {
            node(label: 'vlno-tenfe02-rel') {
              sleep 15
              dir(path: '/usr/local/xrfs_s/xars/linuxXtDevTools/downloads') {
                sh 'mkdir -p $RELEASE_VERSION'
              }

              dir(path: "/usr/local/xrfs_s/xars/linuxXtDevTools/downloads/$RELEASE_VERSION") {
                sh 'mkdir -p builds tools'
              ...
              dir(path: "/usr/local/xrfs_s/xars/linuxWS/$PRECREATED_WS_BRANCH") {
                sh 'cp -r $PREVIOUS_RELEASE_VERSION $RELEASE_VERSION'
              }
        ...
        stage('vlpune-samba2 Copy') {
          steps {
            node(label: 'vlno-tenfe01') {
              sleep 15
              dir(path: '/usr/local/vlpune-samba2/downloads/') {
